...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    if (pass1Input === pass2Input) {
      ...
      //Call API for signup
      try {
        const response = await fetch("http://34.148.52.211/register", {
          method: "POST",
          headers: {
  ...
  const AnimatedButton = ({ p }) => {
    ...
    return (
      <button
        ...
        type="submit"
        onClick={animate}
        className={shake ? `shake submit-button` : `submit-button`}
      >
        {p}
      </button>
    );
      ...
      <div className="signup-wrapper">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p className="text-center pt-3">
            Already have an account? <Link to="/login">Jump back in!</Link>
          </p>
        </form>
      </div>
    </div>
